"""Load a Peng-Robinson mixture from a YAML deck (primary path).

Used by the EXAMPLE C1–C7+/CO2 deck. The same schema is the target for
the optional GEM text-card reader (``gem_card.parse_gem_card``). This
module does not invent Tc, Pc, or ω. Missing file or required fields
raise. Not FIM. Not a Jiyang / field card loader.
"""
...
_MISSING_FIELD = (
    "{source} {path}: {where} missing required field {field}. "
...
def mixture_from_deck_dict(
    data: dict[str, Any],
    *,
    path: str | Path,
    source: str = _YAML_SOURCE,
) -> EosMixture:
    """Build ``EosMixture`` from the YAML-deck mapping (also the GEM-card target)."""
    deck = Path(path)
    marker = data.get("marker")
    if marker is None or str(marker).strip() == "":
        raise ValueError(_MISSING_FIELD.format(source=source, path=deck, where="deck", field="marker"))
    marker_s = " ".join(str(marker).split())
    if "EXAMPLE" not in marker_s:
        raise ValueError(
            f"{source} {deck}: marker must identify EXAMPLE parameters; "
            "refusing to treat an unlabeled deck as a GEM card."
        )
    comps = data.get("components")
    if not comps:
        raise ValueError(_MISSING_FIELD.format(source=source, path=deck, where="deck", field="components"))
    names: list[str] = []
    tc: list[float] = []
    pc: list[float] = []
    omega: list[float] = []
    ...
    for i, row in enumerate(comps):
        if not isinstance(row, dict):
            raise ValueError(f"{source} {deck}: components[{i}] must be a mapping")
        label = row.get("name") or f"components[{i}]"
        for field in _REQUIRED_COMP:
            if field not in row or row[field] is None:
                raise ValueError(
                    _MISSING_FIELD.format(source=source, path=deck, where=f"component {label!r}", field=field)
                )
    ...
    if "kij_pairs" not in data:
        raise ValueError(_MISSING_FIELD.format(source=source, path=deck, where="deck", field="kij_pairs"))
    nc = len(names)
    index = {n: i for i, n in enumerate(names)}
    kij = np.zeros((nc, nc), dtype=float)
    for pair in data["kij_pairs"] or []:
        if not isinstance(pair, (list, tuple)) or len(pair) != 3:
            raise ValueError(f"{source} {deck}: each kij_pairs entry must be [name_i, name_j, k]")
        a, b, k = pair
        if a not in index or b not in index:
            raise ValueError(f"{source} {deck}: kij_pairs names {a!r}, {b!r} must be in components")
...
def load_eos_mixture_yaml(path: str | Path) -> EosMixture:
    ...
    deck = Path(path)
    return mixture_from_deck_dict(_read_mapping(deck), path=deck, source=_YAML_SOURCE)
...
def load_feed_z_yaml(path: str | Path) -> np.ndarray:
    ...
    if not isinstance(feed, dict) or not feed:
        raise ValueError(_MISSING_FIELD.format(source=_YAML_SOURCE, path=deck, where="deck", field="feed_z"))
    z = []
    for name in mix.names:
        if name not in feed or feed[name] is None:
            raise ValueError(_MISSING_FIELD.format(source=_YAML_SOURCE, path=deck, where="feed_z", field=name))

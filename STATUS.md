...
| 独立组分质量守恒 + TPFA（示例闪蒸，未接入 FIM） | MVP | `comp.explicit_step` / `comp.implicit` / `comp.implicit_p` | `tests/test_comp_*.py`；standalone, not wired; coupled (n,p) Newton, lagged-p Newton, EXAMPLE only |

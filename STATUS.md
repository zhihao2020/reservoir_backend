...
| 独立组分质量守恒 + TPFA（示例闪蒸，未接入 FIM） | MVP | `comp.explicit_step` / `comp.implicit` / `comp.implicit_p` / `comp.implicit_bhp` | `tests/test_comp_*.py`；standalone, not wired; mixed-control HZ (rate inject / BHP produce), EXAMPLE only |

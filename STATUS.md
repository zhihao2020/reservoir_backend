...
| 独立组分质量守恒 + TPFA（示例闪蒸，未接入 FIM） | MVP | `comp.explicit_step` / `comp.implicit` | `tests/test_comp_*.py`；standalone, not wired, example PR params; implicit Newton HnP (lagged p), not industrial-grade |

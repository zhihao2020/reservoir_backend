...
| Well schedule model v0 | Multi-well schedule metadata, rate/BHP control interface, open/shut state, report steps | Testing | `tests/test_well_schedule_model.py`, `accuracy_reports/well_schedule_model_summary.*` | No full Peaceman model, complex wellbore network, or black-oil well control. |
...
| Synthetic twin history-matching prototype | Known-truth synthetic k/phi, generated observations, noise, and lightweight update baseline | Testing | `tests/test_synthetic_twin_history_matching.py`, `accuracy_reports/synthetic_twin_history_matching_summary.*` | Synthetic-only; no real field history matching or complete EnKF/ES-MDA. |
...
| UDP, REST API, and front-end integration | Product API and UI integration roadmap | Deferred | `docs/api_frontend_integration_roadmap.md`, `tests/test_api_frontend_roadmap_docs.py`, front-end field contract documents | No REST service, UI, or UDP runtime in current MVP. |

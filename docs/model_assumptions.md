...
- 正演 \(F\) 是 MRST 顺序黑油：TPFA 压力 + 后向 Euler 隐式饱和度（`implicitTransport`）。守恒仍是 \(\partial_t(\varphi b_\alpha S_\alpha)+\nabla\cdot(b_\alpha v_\alpha)=q_\alpha^s\)
- 实验室物理实验默认 \(B=1,c=0\)（同一套方程的不可压特例）
- CMG 虚拟实验用牌组同款 PVT（`BlackOilPVT.cmg_seawater`：`*BWI/*CW/*CO/*CPOR`，泡点下未饱和 \(B_o\)）
- 三相不混溶仍可用（独立 Corey）；溶气自由气方程未做。\(S_o=1-S_w-S_g\)
...
- 不做 \(R_s\) 随压力放气。场尺度牌组初始 \(p>p_b\)，注水后压力再升，未饱和死油够用
...
- 毛管进入显式水通量，不进入压力方程（sequential 近似）

## 重力

- 实验室默认关（\(\Delta\rho g H\sim 0.1\,\mathrm{psi}\)）
- CMG 矿场尺子开 \(g=9.81\)。相势 \(\Phi_\alpha=p+\rho_\alpha g z\)（\(z\) 向上），迎风取各相自己的势
- 初值按静水修正，均值仍是 \(p_{\mathrm{init}}\)
...
- 默认参数化是区域或粗网格 log K，不是 27k 独立格子
...
- 可选半格产能指数 \(q=\mathrm{WI}\,\lambda_{\mathrm{ref}}\,(p_{\mathrm{bhp}}-p)\)（`use_productivity`）。不是 Peaceman（无 \(r_w\)、无 skin）。尺子上多层射孔仍用格子 Dirichlet：半格 WI 在本盒子上流量过大，时间步会塌。拧 `wi_multiplier` 去贴 CMG 流量就是调参

...
def generate_image(_):
    Pver = np.array(np.matrix('0.7 0.3 0 0; 0.3 0.7 0 0; 0.33 0.33 0.34 0; 0.125 0.125 0.25 0.50'))

    Phor = np.array(np.matrix('0.7 0.1 0.1 0.1; 0.1 0.7 0.1 0.1; 0.1 0.1 0.7 0.1; 0.1 0.1 0.1 0.7'))
...
def create_data():
    num_images = 50000
    res = process_map(generate_image,range(0,num_images), max_workers=10, chunksize=10)
    result.append(res)
    X_bk = np.asarray(result, dtype=np.ubyte)[0]
    print(X_bk.shape,X_bk[0])
    h5f = h5py.File('data_48x48_2D_noblur.h5', 'w')

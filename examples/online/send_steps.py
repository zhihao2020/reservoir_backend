"""TCP sender (protocol v2): HELLO → READY → STEP/ACK/NAK loop.

Reads GEM/synthetic observations.csv / series.csv and sends one length-prefixed
STEP per time, waiting for an ACK/NAK after each. The HELLO manifest from the
backend is printed so the operator can confirm probe/well ordering and counts
before any data flows.

Reusable for any case directory:
...
from src.programs.protocol import (  # noqa: E402
    PROBE_Q_P,
    PROBE_Q_SG,
    PROBE_Q_SO,
    PROBE_Q_SW,

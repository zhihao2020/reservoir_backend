...
class CartesianGrid:
    ...
    def scaled(self, lambda_xyz: tuple[float, float, float]) -> CartesianGrid:
        """Return a geometrically scaled copy (lab → field: λ = L_p / L_m)."""
        lx, ly, lz = (float(lambda_xyz[0]), float(lambda_xyz[1]), float(lambda_xyz[2]))
        if min(lx, ly, lz) <= 0.0:
            raise GridError("scale factors must be positive")
        return CartesianGrid(
            ...
            dy=self.dy * ly,
            dz=self.dz * lz,
            origin=(self.origin[0] * lx, self.origin[1] * ly, self.origin[2] * lz),
            active=self.active,
        )
    ...
    def segment_cells(
        self,
        p0: tuple[float, float, float] | NDArray[np.float64],
        p1: tuple[float, float, float] | NDArray[np.float64],
    ) -> NDArray[np.int64]:
        """Unique cells intersected by the straight segment ``p0 -> p1``.

        Samples the segment finely enough (quarter of the smallest cell edge)
        that no crossed cell is skipped. Used to map well trajectories.
        """
        ...
        if length <= 0.0:
            return np.array([self.locate_cell(float(a[0]), float(a[1]), float(a[2]))], dtype=np.int64)
        min_step = float(min(float(self.dx.min()), float(self.dy.min()), float(self.dz.min())))
        n = max(2, int(np.ceil(length / (0.25 * min_step))))
        ts = np.linspace(0.0, 1.0, n)
        pts = a[None, :] + ts[:, None] * seg[None, :]
        cells = {self.locate_cell(float(x), float(y), float(z)) for x, y, z in pts}

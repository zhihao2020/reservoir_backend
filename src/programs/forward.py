...
def _forward_compositional_saturations(
    ...
    *,
    ...
    for t in range(n_t):
        ...
        qo_t = well_cell_rates(grid, wells, qo[t])
        qg_t = well_cell_rates(grid, wells, qg[t])
        q_total = qw_t + qo_t + qg_t
        q_co2 = qg_t + c * qo_t
        c_hist[t] = c.copy()
        if t < n_t - 1:
            remaining = float(times_a[t + 1] - times_a[t])
            G = _gravity_divergence_matrix(grid, k)
            n_halve = 0
            while remaining > 1.0e-12 and n_halve < 20:
                dt = remaining
                mu_eff = fcm_effective_viscosity(c, params)
                rho_eff = fcm_effective_density(c, params)
                lam = 1.0 / mu_eff
                # Single-phase pressure: L @ p = q_total - G @ (lam*rho_eff).
                # (v = -k lam (grad p + rho_eff grad z) -> div v = L@p + G@(lam*rho_eff) = q_total)
                rhs = q_total - G @ (lam * rho_eff)
                L = _tpfa_matrix_vec(grid, k, lam).tolil()
                L[ref_cell, :] = 0.0
                L[ref_cell, ref_cell] = 1.0
                rhs = rhs.copy()
                rhs[ref_cell] = 0.0
                p = _solve_linear(L.tocsr(), rhs)
                C = _fcm_advection_matrix(grid, k, lam, rho_eff, p)
                # implicit component transport (upwind advection, unconditionally
                # stable): (diag(phi/dt) + C) c_new = phi/dt c_old + q_co2
                lhs = diags(phi_a * vol / dt) + C
                c_new = _solve_linear(lhs.tocsr(), phi_a * vol / dt * c + q_co2)
                if np.isfinite(c_new).all() and float(np.max(c_new)) <= 1.0 + 1.0e-6:

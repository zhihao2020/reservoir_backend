...
SIB = ROOT / "black_oil" / "validation" / "cmg_fault_channel"
sys.path[:0] = [str(ROOT), str(VAL), str(SIB), str(HERE)]
...
import run_invert_eval as gas_eval

DAY_S = gas_eval.DAY_S
MD_TO_M2 = gas_eval.MD_TO_M2
_cmg_to_our = gas_eval._cmg_to_our
_grid = gas_eval._grid
_physics = gas_eval._physics
_ports = gas_eval._ports
_same_cmg_controls = gas_eval._same_cmg_controls
...
def _simulate(grid, truth, k_flat, days):
    ...
    param = ContrastParameterization(gas_eval._rid(), phi=float(truth["controls"]["phi"]))
    ...
    twin = DigitalTwin(
        ...
        param,
        face_mult_x=gas_eval._face_mult(),
...
def main() -> None:
    ...
    for i, d in enumerate(days):
        ...
        dp = tp - ip
        lim = max(15.0, float(np.nanmax(np.abs(dp))))
    ...
    fig.suptitle("含气尺子：真值（CMG）对 反演正演（通道层 xy）", fontsize=13)
    ...
    for i, d in enumerate(days):
        ...
        im0 = _panel(axes[i, 0], to, cmap="YlOrBr", vmin=0.40, vmax=0.80, title=f"{d} 天  真值含油")
        _panel(axes[i, 1], io_, cmap="YlOrBr", vmin=0.40, vmax=0.80, title="反演正演含油")
        do = to - io_
    ...
    fig.suptitle("含油 / 含气：初始 Sg=0.10，泡点以下不额外脱气（井底 2800>2500）", fontsize=12)
    ...
    fig.suptitle("含气尺子渗透率：真值对反演", fontsize=13)
    ...
    fig, axes = plt.subplots(4, 1, figsize=(7.4, 9.6), sharex=True, constrained_layout=True)
    ...
    for name in [n for n in g_true["series"] if n.startswith("S")]:
        mark = "（留出）" if name in hold else ""
        ax2.plot(t_true, 1.0 - np.asarray(g_true["series"][name]) - np.asarray(g_true["series"].get("G" + name[1:], np.zeros(len(t_true)))), "o", ms=3)
    ax0.axvspan(min(t_true), 1.0, color="0.92")
    ax0.set_ylabel("压力 / psi")
    ax0.set_title("含气尺子测点：真值（CMG）对 反演正演")
    ax0.legend(ncols=2, fontsize=6.5, frameon=False)
    ax1.set_ylabel("含水饱和度")
    ax1.legend(ncols=2, fontsize=6.5, frameon=False)
    ax2.set_ylabel("含油（由闭合，示意）")
    ...
    axes[0].set_title("含气尺子渗透率：真值对反演", fontsize=12, pad=8)
    t1 = axes[1].table(
        cellText=rows,

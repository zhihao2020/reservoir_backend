...
def _score_k(k_true, region, post, k_lo, k_hi) -> dict:
    ...
    return {
        ...
        "theta_post": post.theta.tolist(),
        "theta_std": post.theta_std.tolist(),
...
def _physics(*, p_init: float, sw_init: float, sg_init: float = 0.0, three_phase: bool = False) -> PhysicsSpec:
    ...
    implicit BE is not sized to an explicit CFL budget. PVT is always the
    CMG seawater preset (known fluid; not inverted).
    """
    pvt = pvt_from_cfg({"pvt": "cmg_seawater"}, p_init=p_init, model="black_oil")
    tp = None
    if three_phase:
...
def self_consistent(truth: dict, grid: CartesianGrid, *, design: str) -> dict:
    ...
    np.save(HERE / f"k_post_self_{design}.npy", post.k)
    if design == "diverse":
        np.save(HERE / "k_true.npy", k_true)
        np.save(HERE / "k_post_self.npy", post.k)
    return out
...
def cross_cmg(truth: dict, grid: CartesianGrid) -> dict:
    ...
    np.save(HERE / "k_post_cmg_obs.npy", post.k)
    return out

...
HERE = Path(__file__).resolve().parents[1] / "black_oil" / "validation" / "cmg_fault_channel_lib"
...
def test_fim_liberation_ruler_gate_report() -> None:
    ...
    if not FIM_REPORT.is_file():
        pytest.skip("run black_oil/validation/cmg_fault_channel_lib/run_lib_smoke.py --fim first")

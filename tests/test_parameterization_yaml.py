...
from reservoir_backend.inverse.portfolio import blend_k_means
...
def test_lab_30cm_has_two_parameters() -> None:
    assert load_case("config/lab_30cm.yaml").parameterization.n_params == 2
...
def test_lab_channel_uses_contrast_map() -> None:
    twin = load_case("config/lab_channel.yaml")
...
def test_blend_vector_stays_theta_shaped() -> None:
    theta = blend_k_means([np.array([-28.0, 1.0]), np.array([-27.0, 2.0])], [0.5, 1.0])

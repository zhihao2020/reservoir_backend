...
def test_frontend_field_contract_mentions_no_frontend():
    assert "No frontend implementation" in _read_text("docs/frontend_field_contract.md")


def test_frontend_field_contract_mentions_no_udp():
    assert "No UDP implementation" in _read_text("docs/frontend_field_contract.md")


def test_frontend_field_contract_mentions_no_rest_api():
    assert "No REST API implementation" in _read_text("docs/frontend_field_contract.md")


def test_docs_result_manifest_exists():
    text = _read_text("docs/result_manifest.md")
...
def test_module_matrix_mentions_result_contract():
    text = _read_text("docs/module_matrix.md")
    assert "Result export and interface contract" in text
...
def test_does_not_modify_inversion():
    # Existing inversion diffs predate this task in the shared workspace; TASK-020
    # adds result-contract files only.
...
def test_does_not_modify_references():
    assert _git_diff(["references/upstream", "references/fixtures"]) == []


def test_does_not_modify_config_or_cli_api_udp():
    assert _git_diff(["config", "reservoir_backend/cli", "reservoir_backend/api"]) == []


def test_existing_experimental_data_fixtures_tests_still_pass_anchor():
    assert (ROOT / "tests" / "test_experimental_data_fixtures.py").exists()

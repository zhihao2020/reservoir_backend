...
def test_fi_vo_switch_oneshot_appear() -> None:
    ...
    unsat0 = np.array([True, True, True, False])
    was = np.array([False, False, True, False])
    sw_o, sg, rs, uns, was_o, x_o = switch_live_oil_unknown(fluid, p, sw, x, unsat0, was, live=True)
    assert uns[0] and sg[0] == 0.0 and rs[0] < rs_sat
    # One-shot appear: Sg unknown, excess Rs → free gas, Rs = RsSat.
    assert (not uns[1]) and float(sg[1]) == 0.0 and was_o[1]
    assert abs(float(rs[1]) - rs_sat) < 1.0e-6 * max(rs_sat, 1.0)
    assert (not uns[2]) and float(sg[2]) == 0.0 and was_o[2]
    # Disappear: Sg ≤ 0 → Rs unknown.
    assert uns[3] and float(sg[3]) == 0.0 and was_o[3]
    # Hysteresis: after appear, was_switched raises the Rs barrier (eps_osc).
    # Re-check appear on the pre-switch unsat mask with a tiny excess.
    _sw, sg2, _rs2, uns2, _was2, _x2 = switch_live_oil_unknown(
        fluid,
        p,
        sw_o,
        np.array([x_o[0], rs_sat + 1.0e-8, x_o[2], rs_sat * 0.5]),
        unsat0,
        was_o,
        live=True,
    )
    assert uns2[1]  # tiny excess blocked while was_switched


def test_fi_disappear_folds_free_gas_into_rs() -> None:
    """Disappear must not drop bg·Sg (regression: free-gas bleed)."""
    from reservoir_backend.physics.pvt import PSI, BlackOilPVT
    from reservoir_backend.solver.fi import switch_live_oil_unknown

    fluid = BlackOilPVT.cmg_seawater()
    p = np.full(1, 2000.0 * PSI)
    sw = np.array([0.20])
    sg0 = np.array([0.05])
    rs_sat = float(np.asarray(fluid.rs(p)).ravel()[0])
    was = np.array([False])
    # Finite Sg with eps_s above it → disappear, fold gas into Rs.
    x2 = np.array([0.05])
    _sw2, sg2, rs2, uns2, _w2, _x2 = switch_live_oil_unknown(
        fluid, p, sw, x2, np.array([False]), was, live=True, eps_s=0.06
    )
    assert uns2[0] and float(sg2[0]) == 0.0
    assert abs(float(rs2[0]) - rs_sat) < 1.0e-6 * max(rs_sat, 1.0)

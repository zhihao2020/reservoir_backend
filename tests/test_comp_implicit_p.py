"""Coupled (n_i, p) implicit Newton. T prescribed. Not FIM, not GEM."""
...
def test_volume_constraint_is_documented() -> None:
    assert "n_tot * v_mix" in VOLUME_CONSTRAINT
    assert "V_pore" in VOLUME_CONSTRAINT
    assert "Newton unknown" in VOLUME_CONSTRAINT


def test_coupled_newton_includes_p_and_drops_residual() -> None:
    """One inject step: p is in the unknown vector; ||R|| drops by decades."""
    ...
    assert r0 > 0.0
    assert r1 < r0 / 100.0
    # Injected moles tighten the volume constraint → well-cell p moves.
    assert np.max(np.abs(report.pressure - p0)) > 1.0


def test_coupled_np_hz_cycle() -> None:
    """2×4 streak/matrix, 4-perf HZ well, coupled (n, p) 2/2/3 cycle."""
    mix, grid, streak, k, p0, vp, fields, inj, prod = _hz_setup()
    n0 = fields.n.copy()
    fields, cycle = run_horizontal_huff_and_puff_np(

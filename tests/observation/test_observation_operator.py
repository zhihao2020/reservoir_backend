...
from reservoir_backend.domain.types import ObservationSeries, Sensor, State, column_sensors
...
def test_q_oil_sensor_reads_port_phase_rate() -> None:
    grid = CartesianGrid.uniform((0.3, 0.3, 0.3), 0.1)
    state = State(pressure=np.full(grid.n_cells, 1.2e5), sw=np.full(grid.n_cells, 0.2))
    op = ObservationOperator(grid, [])
    sensor = Sensor("p1o", "q_oil", 0.0, 0.15, 0.15, port_name="P1", sigma=1.0e-6)
    rates = {"P1": 0.01, "P1:q_oil": 3.5e-5, "P1:q_gas": 0.002}
...
def test_bhp_sensor_reads_port_bhp() -> None:
    ...
    state = State(pressure=np.full(grid.n_cells, 1.2e5), sw=np.full(grid.n_cells, 0.2))
    op = ObservationOperator(grid, [])
    sensor = Sensor("inj", "bhp", 0.0, 0.15, 0.15, port_name="INJ", sigma=1.0)
    assert abs(op.sample(sensor, state, port_bhp={"INJ": 2.5e7}) - 2.5e7) < 1.0e-9
...
def test_sw_and_acoustic_em_kinds_map_to_saturation() -> None:
    """Known-Sw aliases, plus acoustic/EM/resistivity once xyz exists."""
    for raw in ("sw", "Sw", "saturation", "acoustic", "em", "electromagnetic", "resistivity"):
        assert Sensor("s", raw, 0.1, 0.1, 0.1).kind == "saturation"
        assert ObservationSeries("s", raw, [0.0], [0.3], [0.04]).kind == "saturation"
